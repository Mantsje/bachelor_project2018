module grammar::rewrite_grammar::RewriteRules
...
set[Production] removePriority(Symbol s, list[Production] l) {
	return ( {} | it + rewriteRule(p) | p <- l);
}
...
Symbol removeLabel(Symbol s) {
	if (\label(_, Symbol in_s) := s) return removeLabel(in_s);
	return s;
}
...
map[Symbol, set[Production]] mapRewrittenRules(Grammar g) {
	set[Production] all_rules = {g.rules[sym] | sym <- g.rules};
	set[Production] new_rules = ({} | it + rewriteRule(p) | p <- all_rules);
	relset = ({} | it + <lh,p> | p:\prod(Symbol lh, _, _) <- new_rules);
	return index(relset);
}