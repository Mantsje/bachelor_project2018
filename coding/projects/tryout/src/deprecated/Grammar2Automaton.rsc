...
map[Symbol, StateMachine] generateNFAPerComponent(map[Symbol, StateMachine] nfas, StateMap statemap, set[Symbol] symset, Symbol startSym, set[set[Token]] components) {
	...
	for (comp <- components) {
		...
		for (sym <- comp) new_nfas[sym] = \sm-nfa(merged.transitions, nfas[sym].startState, nfas[sym].finalStates);
	}
	return new_nfas;
}
...
//TODO: look into getRewritten rules here? is mohri's algorithm needed: line 206?
map[Symbol, StateMachine] generateNamedDFAPerSymbol(Grammar g) {
	<startSymbol, rules, comps> = preprocessGrammar(g);
	rules = getRewrittenRules(g);
	set[Symbol] symset = generateSymbolSet(rules);
	map[Symbol, StateMachine] symbol_nfas = generateNFAPerSymbol(symset, rules); 
	StateMap statemap = generateStateMap(symset);
	return (n:NFA2DFAPreserveNames(symbol_nfas[n], statemap, n, false) | n <- symbol_nfas);
}
...
StateMachine generateSuperDFA(Grammar g) {
	...
	StateMap statemap = generateStateMap(symset);
	map[Symbol, StateMachine] symbol_nfas = generateNFAPerSymbol(symset, rules);
	symbol_nfas = (n:NFA2DFAPreserveNames(symbol_nfas[n], statemap, n, true) | n <- symbol_nfas);
