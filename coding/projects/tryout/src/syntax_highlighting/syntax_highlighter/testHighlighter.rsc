...
void main() {
	...
	mat_num = 	\match(\str-regex("{{Num}}"), \scope("constant.numeric.integer.st"), \setact([con_id]));
	con_num = 	\context-no-scope("Context-Num", {}, {});
	mat_id = 	\match(\str-regex("{{Id}}"), \scope("keyword.controlflow.st"), \setact([con_num])); 
	con_id.matches += {mat_id};
	con_num.matches += {mat_num};
	con_main = 	\main-no-scope({}, {con_id, con_num});
	hl = \highlighter("Stat", {"st"}, {id, number}, {con_id, con_num, con_main});
